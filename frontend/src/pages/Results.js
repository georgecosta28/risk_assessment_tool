...
export default function Results() {
  ...
  const { risk_score, risk_level, details } = location.state || {};
  ...
  const { object: categoryAverages, array: categoryChartData } = getCategoryAverages();
  ...
  return (
    <div style={{ padding: '30px', fontFamily: 'Arial, sans-serif' }}>
      ...
      <p><strong>Risk Score:</strong> {risk_score.toFixed(2)}</p>
      <p><strong>Risk Level:</strong> <span style={{ color: colorMap[risk_level], fontWeight: 'bold' }}>{risk_level}</span></p>

      <div style={{ marginTop: '30px' }}>
        <h3>Risk Level Distribution</h3>
        <RiskBarChart details={details} />
      </div>

      <div style={{ marginTop: '40px' }}>
        <h3>Risk Matrix</h3>
        <RiskMatrixChart details={details} />
      </div>
          ...
          <tbody>
            {details.map((item, idx) => (
              <tr key={idx}>
                ...
                <td style={{ ...tdStyle, fontStyle: 'italic', color: '#555' }}>{RECOMMENDATIONS[item.question - 1]}</td>
              </tr>
            ))}
          </tbody>
        </table>
